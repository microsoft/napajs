...
void AllocatorWrap::DeallocateCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    JS_ENSURE(
        isolate, result.second, "Unable to cast \"handle\" to pointer. Please check if it's in valid handle format.");
...
void AllocatorWrap::GetTypeCallback(v8::Local<v8::String> /*propertyName*/,
    ...
    args.GetReturnValue().Set(
        v8_helpers::MakeV8String(isolate, allocator != nullptr ? allocator->GetType() : "<unloaded>"));
}