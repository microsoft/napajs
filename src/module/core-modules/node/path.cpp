...
void path::Init(v8::Local<v8::Object> exports) {
    ...
    (void)exports->CreateDataProperty(
        context, v8_helpers::MakeV8String(isolate, "sep"), v8_helpers::MakeV8String(isolate, platform::DIR_SEPARATOR));
}
...
void NormalizeCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    CHECK_ARG(
        isolate, args.Length() == 1 && args[0]->IsString(), "path.normalize requires 1 string parameter of file path.");
...
void ResolveCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    CHECK_ARG(isolate, args.Length() > 0, "path.resolve requires at least one string parameters.");

    auto path = filesystem::CurrentDirectory();

    for (int i = 0; i < args.Length(); ++i) {
        CHECK_ARG(isolate, args[i]->IsString(), "path.resolve doesn't accept non-string argument.");
...
void JoinCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    CHECK_ARG(isolate, args.Length() > 0 && args[0]->IsString(), "path.join requires at least one string parameters.");

    v8::String::Utf8Value basePath(args[0]);
    auto path = filesystem::Path(*basePath);

    for (int i = 1; i < args.Length(); ++i) {
        CHECK_ARG(isolate, args[i]->IsString(), "path.join doesn't accept non-string argument.");
...
void DirnameCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    CHECK_ARG(
        isolate, args.Length() == 1 && args[0]->IsString(), "path.dirname requires 1 string parameter of file path.");
...
void BasenameCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    CHECK_ARG(isolate, args[0]->IsString(), "path.basename requires a string parameter of file path.");

    v8::String::Utf8Value utf8Path(args[0]);
    auto fileName = filesystem::Path(*utf8Path).Filename().String();
    if (args.Length() == 2) {
        CHECK_ARG(isolate, args[1]->IsString(), "path.basename requires a string as 2nd parameter of extension.");
...
void ExtnameCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    CHECK_ARG(
        isolate, args.Length() == 1 && args[0]->IsString(), "path.extname requires 1 string parameter of file path.");
