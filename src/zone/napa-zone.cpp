...
/// <summary> Load 'napajs' module during bootstrap. We use relative path to decouple from how module will be published.
/// </summary>
static const std::string NAPAJS_MODULE_PATH =
    filesystem::Path(dll::ThisLineLocation()).Parent().Parent().Normalize().String();
static const std::string BOOTSTRAP_SOURCE =
    "require('" + utils::string::ReplaceAllCopy(NAPAJS_MODULE_PATH, "\\", "\\\\") + "');";
...
NapaZone::NapaZone(const settings::ZoneSettings& settings) : _settings(settings) {
    ...
    Broadcast(BOOTSTRAP_SOURCE, [&promise](ResultCode code) { promise.set_value(code); });
...
void NapaZone::Broadcast(const std::string& source, BroadcastCallback callback) {
    ...
    auto callOnce = [ this, source, callback = std::move(callback), counter ](napa_result_code code) {
        if (--(*counter) == 0) {
            NAPA_DEBUG(
                "Zone", "Finishing broadcast script \"%s\" to zone \"%s\"", source.c_str(), _settings.id.c_str());
            callback(code);
        }
    };
...
void NapaZone::Execute(const FunctionSpec& spec, ExecuteCallback callback) {
    ...
    if (spec.options.timeout > 0) {
        task = std::make_shared<TimeoutTaskDecorator<CallTask>>(
            std::chrono::milliseconds(spec.options.timeout), std::make_shared<CallContext>(spec, std::move(callback)));
    } else {
        task = std::make_shared<CallTask>(std::make_shared<CallContext>(spec, std::move(callback)));
    }

    NAPA_DEBUG("Zone",
               "Execute function \"%s.%s\" on zone \"%s\"",
               spec.module.data,
               spec.function.data,
               _settings.id.c_str());
