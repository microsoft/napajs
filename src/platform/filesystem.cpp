...
namespace filesystem {
    ...
    namespace {
        ...
        std::string ParseDriveSpec(const std::string& path, bool* hasUncPrefix = nullptr) {
            ...
            if (path.size() < driveSpecStart + 2 || !IsLetter(path[driveSpecStart]) ||
        ...
        std::string FormatPathString(bool hasUncPrefix,
    ...
    Path& Path::operator/=(const Path& path) { return Append(path); }
    ...
    int Path::Compare(const Path& path) const { return _pathname.compare(path._pathname); }

    /// <summary> Get unnormalized path string. </summary>
    /// <remarks> Use GenericForm() to always use '/' as delimiter. </summary>
    const std::string& Path::String() const { return _pathname; }

    /// <summary> Get unnormalized path string buffer. </summary>
    const CharType* Path::c_str() const { return _pathname.c_str(); }
    ...
    Path Path::DriveSpec() const { return Path(ParseDriveSpec(_pathname)); }
    ...
    bool Path::HasUncPrefix() const { return ParseUncPrefix(_pathname); }
    ...
    bool operator==(const Path& lhs, const Path& rhs) { return lhs.Compare(rhs) == 0; }
    ...
    PathIterator::PathIterator(Path path) : _base(std::move(path)) {
#ifdef SUPPORT_POSIX
    ...
    const Path& PathIterator::operator*() const { return _currentPath; }
