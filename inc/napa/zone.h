...
public:
    ...
    ~Zone() { napa_zone_release(_handle); }
    ...
    const std::string& GetId() const { return _zoneId; }
    ...
    void Broadcast(const std::string& source, BroadcastCallback callback) {
        ...
        napa_zone_broadcast(_handle,
                            ...
                            [](napa_result_code code, void* context) {
                                ...
                                std::unique_ptr<BroadcastCallback> callback(
                                    reinterpret_cast<BroadcastCallback*>(context));
    ...
    ResultCode BroadcastSync(const std::string& source) {
        ...
        Broadcast(source, [&prom](ResultCode code) { prom.set_value(code); });
    ...
    Result ExecuteSync(const FunctionSpec& spec) {
        ...
        Execute(spec, [&prom](Result result) { prom.set_value(std::move(result)); });
