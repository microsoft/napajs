...
function shouldFail<T>(func: () => Promise<T>) {
    return func().then((value: T) => { assert(false, "Failure was expected."); },
                       (reason: any) => {
    ...
    function() {
        ...
        describe("get",
                 () => {
                     ...
                     it('@node: id not existed', () => { assert.throws(() => { napa.zone.get('zonex'); }); });

                     it('@napa: zone not existed',
                        () => {
                            shouldFail(() => { return napaZone1.execute(`${napaLibPath}/zone`, 'get', [ 'zonex' ]); });
                        });
                 });

        describe("currentZone",
                 () => {
                     it('@node', () => { assert.strictEqual(napa.zone.current.id, 'node'); });
        ...
        describe(
            ...
            () => {
                ...
                it('@node: -> napa zone with JavaScript code', () => { return napaZone1.broadcast("var state = 0;"); });
                ...
                it('@node: bad JavaScript code',
                   () => { return shouldFail(() => { return napaZone1.broadcast("var state() = 0;"); }); });

                it('@napa: bad JavaScript code',
                   () => {
                       return shouldFail(() => {
                           return napaZone1.execute(
                               napaZoneTestModule, "broadcast", [ "napa-zone2", "var state() = 0;" ]);
                       });
                   });

                it('@node: -> node zone throw runtime error',
                   () => { return shouldFail(() => { return napa.zone.current.broadcast("throw new Error();"); }); });

                it('@node: -> napa zone throw runtime error',
                   () => { return shouldFail(() => { return napaZone1.broadcast("throw new Error();"); }); });

                it('@napa: -> napa zone throw runtime error',
                   () => {
                       return shouldFail(() => {
                           return napaZone1.execute(
                               napaZoneTestModule, "broadcast", [ "napa-zone2", "throw new Error();" ]);
                       });
                   });
                ...
                it('@node: -> node zone with anonymous function',
                   () => {
                       return napa.zone.current.broadcast((input: string) => { console.log(input); },
                                                          [ 'hello world' ]);
                   });

                it('@node: -> napa zone with anonymous function',
                   () => {
                       return napaZone1.broadcast((input: string) => { console.log(input); }, [ 'hello world' ]);
                   });
                ...
                it.skip('@node: -> node zone with transportable args',
                        () => {
                            return napa.zone.current.broadcast((allocator: any) => { console.log(allocator); },
                ...
                it.skip('@node: -> napa zone with transportable args',
                        () => {
                            return napaZone1.broadcast((allocator: any) => { console.log(allocator); },
                                                       [ napa.memory.crtAllocator ]);
                        });
                ...
                it('@node: -> node zone with anonymous function having closure (should fail)',
                   () => {
                       return shouldFail(
                           () => { return napa.zone.current.broadcast(() => { console.log(napaZone1.id); }); });
                   });

                it('@node: -> napa zone with anonymous function having closure (should fail)',
                   () => {
                       return shouldFail(() => { return napaZone1.broadcast(() => { console.log(napaZone1.id); }); });
                   });
        ...
        `;
                ...
                it('@node: -> node zone with global function name',
                   () => {
                       return napa.zone.current.execute("", "foo", [ 'hello world' ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 'hello world'); });
                   });
                ...
                it('@napa: -> napa zone with global function name',
                   () => {
                       return napaZone1.execute(napaZoneTestModule,
                                                'execute',
                                                [ "napa-zone2", "", "foo", [ 'hello world' ] ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 'hello world'); });
                   });
                ...
                it('@node: -> node zone with global function name not exists',
                   () => {
                       return shouldFail(() => { return napa.zone.current.execute("", "foo1", [ 'hello world' ]); });
                   });

                it('@node: -> napa zone with global function name not exists',
                   () => { return shouldFail(() => { return napaZone1.execute("", "foo1", [ 'hello world' ]); }); });
                ...
                it('@napa: -> napa zone with module function name',
                   () => {
                       return napaZone1.execute(napaZoneTestModule,
                                                'execute',
                                                [ "napa-zone2", napaZoneTestModule, "bar", [ 'hello world' ] ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 'hello world'); });
                   });
                ...
                it('@napa: -> node zone with module function name',
                   () => {
                       return napaZone1.execute(napaZoneTestModule,
                                                'execute',
                                                [ "node", napaZoneTestModule, "bar", [ 'hello world' ] ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 'hello world'); });
                   });
                ...
                it('@node: -> napa zone with module function name: module is a function',
                   () => {
                       return napaZone1.execute(path.resolve(__dirname, "./napa-zone/function-as-module"),
                                                "",
                                                [ 'hello world' ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 'hello world'); });
                   });
                ...
                it('@node: -> node zone with module not exists',
                   () => {
                       return shouldFail(() => { return napa.zone.current.execute("abc", "foo1", [ 'hello world' ]); });
                   });

                it('@node: -> napa zone with module not exists',
                   () => { return shouldFail(() => { return napaZone1.execute("abc", "foo1", [ 'hello world' ]); }); });
                ...
                it('@napa: -> napa zone with module function not exists',
                   () => {
                       return shouldFail(() => {
                           return napaZone1.execute(
                               napaZoneTestModule, 'execute', [ "napa-zone1", napaZoneTestModule, "foo1", [] ]);
                       });
                   });
                ...
                it('@napa: -> node zone with module function not exists',
                   () => {
                       return shouldFail(() => {
                           return napaZone1.execute(
                               napaZoneTestModule, 'execute', [ "node", napaZoneTestModule, "foo1", [] ]);
                       });
                   });

                it('@node: -> node zone with anonymous function',
                   () => {
                       return napa.zone.current.execute((input: string) => { return input; }, [ 'hello world' ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 'hello world'); });
                   });

                it('@node: -> napa zone with anonymous function',
                   () => {
                       return napaZone1.execute((input: string) => { return input; }, [ 'hello world' ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 'hello world'); });
                   });
                ...
                it('@node: -> napa zone with anonymous function having closure (should fail)',
                   () => { return shouldFail(() => { return napaZone1.execute(() => { return napaZone1; }); }); });

                it('@napa: -> napa zone with anonymous function having closure (should fail)',
                   () => {
                       return shouldFail(() => {
                           return napaZone1.execute(
                               napaZoneTestModule, 'executeTestFunctionWithClosure', [ "napa-zone2" ]);
                       });
                   });
                ...
                it('@node: -> node zone with transportable returns',
                   () => {
                       return napa.zone.current.execute((allocator: napa.memory.Allocator) => { return allocator; },
                                                        [ napa.memory.crtAllocator ])
                ...
                it('@node: -> napa zone with transportable returns',
                   () => {
                       return napaZone1.execute((allocator: napa.memory.Allocator) => { return allocator; },
                                                [ napa.memory.crtAllocator ])
                           .then((result: napa.zone.Result) => {
                               assert.deepEqual(result.value.handle, napa.memory.crtAllocator.handle);
                           });
                ...
                it.skip('@napa: -> napa zone with timeout and succeed',
                        () => {
                            return napaZone1.execute(napaZoneTestModule,
                                                     'executeTestFunctionWithTimeout',
                                                     [ "napa-zone2", 1 ],
                                                     { timeout : 100 });
                        });
