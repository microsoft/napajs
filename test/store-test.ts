...
describe('napajs/store',
         function() {
             ...
             it('@napa: store.get', () => { napaZone.execute(NAPA_ZONE_TEST_MODULE, "getStoreTest"); });
             ...
             it('simple types: set in napa, get in node',
                async() => {
                    ...
                    assert.deepEqual(store1.get('d'), { a : 1, b : 1 });
                });
             ...
             it('transportable types: set in node, get in napa',
                () => {
                    ...
                    napaZone.execute(
                        NAPA_ZONE_TEST_MODULE, "storeVerifyGet", [ 'store1', 'b', napa.memory.defaultAllocator ]);
                });

             it('transportable types: set in napa, get in napa',
                async() => {
                    // We have to compare handle in this case, since napa.memory.defaultAllocator retrieved from napa
                    // zone will have 2+ refCount.
                    await napaZone.execute(
                        NAPA_ZONE_TEST_MODULE, "storeSet", [ 'store1', 'e', napa.memory.defaultAllocator ]);
                    napaZone.execute(NAPA_ZONE_TEST_MODULE,
                                     "storeGetCompareHandle",
                                     [ 'store1', 'e', napa.memory.defaultAllocator.handle ]);
                });
