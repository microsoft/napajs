...
describe(
    ...
    function() {
        ...
        describe('resolve',
                 function() {
                     // TODO: support correct __dirname in anonymous function and move tests from 'resolution-tests.js'
                     // here.
                     it('require.resolve',
                             ...
                             it('platform',
                                () => {
                                    return napaZone.execute(() => {
                                        ...
                                        assert(process.platform == 'win32' || process.platform == 'darwin' ||
                             ...
                             it('writeFileSync',
                                () => { return napaZone.execute(
                                                           (thisFilePath: string) => {
                                                               ...
                                                               fs.writeFileSync(thisFilePath + '/module/test-file',
                                                                                'test');
                                                               assert.equal(
                                                                   fs.readFileSync(thisFilePath + '/module/test-file'),
                                                                   'test');
                                                           },
                                                           [ __dirname ])
                                            .then(() => {
                             ...
                             it('readFileSync',
                                () => { return napaZone.execute(
                                                           (thisFilePath: string) => {
                                                               ...
                                                               assert.deepEqual(fs.readdirSync(testDir).sort(),
                                                                                [ '1', '2' ]);
                                                           },
                                                           [ __dirname ])
                                            .then(() => {
                                ...
                                () => {
                                    return napaZone.execute(() => {
                                        ...
                                        if (process.platform == 'win32') {
                                            ...
                                            assert.equal(path.resolve("abc", "efg", "../hij", "./xyz.txt"),
                             ...
                             it('join',
                                () => {
                                    return napaZone.execute(() => {
                                        ...
                                        if (process.platform == 'win32') {
                                            assert.equal(path.join("/foo", "bar", "baz/asdf", "quux", ".."),
                                                         "\\foo\\bar\\baz\\asdf");
                                        } else {
                                            assert.equal(path.join("/foo", "bar", "baz/asdf", "quux", ".."),
                                                         "/foo/bar/baz/asdf");
                                ...
                                () => {
                                    return napaZone.execute(() => {
                                        ...
                                        if (process.platform == 'win32') {
                                            ...
                                            assert.equal(path.relative("/test/a", "a.txt"),
                                                         "../.." + process.cwd() + "/a.txt");
                             ...
                             it('type',
                                () => {
                                    return napaZone.execute(() => {
                                        ...
                                        assert(os.type() == "Windows_NT" || os.type() == "Darwin" ||
                                               os.type() == "Linux");
                                    });
                                });
                         });
                   ...
                   () => {
                       return napaZone.execute(
                                          (thisFilePath: string) => {
                                              var assert = require("assert");
                                              var napaModule =
                                                  require(thisFilePath + '/module/addon/build/simple-napa-addon.napa');

                                              var obj = napaModule.createSimpleObjectWrap();
                                              obj.setValue(3);

                                              var promise = new Promise((resolve) => {
                                                  obj.postIncrementWork((newValue: number) => { resolve(newValue); });
                                              });
                                          ...
                                          [ __dirname ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 4); });
                   });
                   ...
                   () => {
                       return napaZone.execute(
                                          (thisFilePath: string) => {
                                              ...
                                              var promise = new Promise((resolve) => {
                                                  obj.doIncrementWork((newValue: number) => { resolve(newValue); });
                                              });
                                          ...
                                          [ __dirname ])
                           .then((result: napa.zone.Result) => { assert.equal(result.value, 9); });
                   });
            });
    });