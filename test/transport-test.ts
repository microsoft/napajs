    ...
    () => {
        ...
        describe('TransportContext',
                 () => {
                     ...
                     it('#saveShared', () => { tc.saveShared(allocator); });
                     ...
                     it('#sharedCount', () => { assert.equal(shareable.refCount, 3); });
                 });
                 ...
                 () => {
                     it('#IsTransportable',
                        () => {
                            ...
                            assert(
                                napa.transport.isTransportable({ a : 1, b : new t.CanPass(napa.memory.crtAllocator) }));
        ...
        describe(
            'Marshall/Unmarshall',
            () => {
                it('@node: simple types', () => { t.simpleTypeTransportTest(); });

                it('@napa: simple types', () => { napaZone.execute(NAPA_ZONE_TEST_MODULE, "simpleTypeTransportTest"); })
                    .timeout(3000);

                it('@node: JS transportable', () => { t.jsTransportTest(); });

                it('@napa: JS transportable', () => { napaZone.execute(NAPA_ZONE_TEST_MODULE, "jsTransportTest"); });

                it('@node: addon transportable', () => { t.addonTransportTest(); });

                it('@napa: addon transportable',
                   () => { napaZone.execute(NAPA_ZONE_TEST_MODULE, "addonTransportTest"); });

                it('@node: function transportable', () => { t.functionTransportTest(); });

                it('@napa: function transportable',
                   () => { napaZone.execute(NAPA_ZONE_TEST_MODULE, "functionTransportTest"); });

                it('@node: composite transportable', () => { t.compositeTransportTest(); });

                it('@napa: composite transportable',
                   () => { napaZone.execute(NAPA_ZONE_TEST_MODULE, "compositeTransportTest"); });

                it('@node: non-transportable', () => { t.nontransportableTest(); });
