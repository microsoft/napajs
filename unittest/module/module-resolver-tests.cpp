...
ModuleResolver& GetModuleResolver() {
    ...
    static bool setNodePath = []() {
        ...
        oss << (filesystem::CurrentDirectory() / "resolve-env").String() << platform::ENV_DELIMITER
...
void ResolveIt(const char* module, const char* expected, ModuleType type) {
...
void ResolveIt(const char* module, const filesystem::Path& expected, ModuleType type) {
    auto detail = GetModuleResolver().Resolve(module);
...
void ResolveIt(const char* module, const filesystem::Path& expected, ModuleType type, const filesystem::Path& package) {
    auto detail = GetModuleResolver().Resolve(module);
...
TEST_CASE("resolve node modules correctly.", "[module-resolver]") {
    ...
    SECTION("resolve as a file") {
        ...
        ResolveIt((currentPath / "resolve-file").c_str(), currentPath / "resolve-file", ModuleType::JAVASCRIPT);
    }
    ...
    SECTION("resolve from node_modules") {
        ...
        ResolveIt(
            "resolver-nm-js", currentPath / "node_modules" / "resolver-nm-js" / "index.js", ModuleType::JAVASCRIPT);

        // A directory with index.json exists.
        ResolveIt(
            "resolver-nm-json", currentPath / "node_modules" / "resolver-nm-json" / "index.json", ModuleType::JSON);

        // A directory with index.napa exists.
        ResolveIt(
            "resolver-nm-napa", currentPath / "node_modules" / "resolver-nm-napa" / "index.napa", ModuleType::NAPA);
    }
    ...
    SECTION("resolve from NODE_PATH") {
        ...
        // Starts with "./" and a javascript file exists.
        ResolveIt(
            "./resolve-env-file-js", currentPath / "resolve-env" / "resolve-env-file-js.js", ModuleType::JAVASCRIPT);

        // Starts with "./" and a json file exists.
        ResolveIt(
            "./resolve-env-file-json", currentPath / "resolve-env" / "resolve-env-file-json.json", ModuleType::JSON);

        // Starts with "./" and a binary file exists.
        ResolveIt(
            "./resolve-env-file-napa", currentPath / "resolve-env" / "resolve-env-file-napa.napa", ModuleType::NAPA);

        // Starts with "../" and a file exists.
        ResolveIt("../resolve-env/resolve-env-file",
                  currentPath / "resolve-env" / "resolve-env-file",
                  ModuleType::JAVASCRIPT);
        ...
        // Starts with "./" and a directory with index.js exists.
        ResolveIt(
            "./resolver-env-js", currentPath / "resolve-env" / "resolver-env-js" / "index.js", ModuleType::JAVASCRIPT);

        // Starts with "./" and a directory with index.json exists.
        ResolveIt(
            "./resolver-env-json", currentPath / "resolve-env" / "resolver-env-json" / "index.json", ModuleType::JSON);

        // Starts with "./" and a directory with index.napa exists.
        ResolveIt(
            "./resolver-env-napa", currentPath / "resolve-env" / "resolver-env-napa" / "index.napa", ModuleType::NAPA);
    ...
    SECTION("resolve from parent node_modules") {
        // A file exists.
        ResolveIt(
            "resolve-cc-file", currentPath / "child" / "node_modules" / "resolve-cc-file", ModuleType::JAVASCRIPT);

        // A javascript file exists.
        ResolveIt("resolve-cc-file-js",
                  currentPath / "child" / "node_modules" / "resolve-cc-file-js.js",
                  ModuleType::JAVASCRIPT);

        // A json file exists.
        ResolveIt("resolve-cc-file-json",
                  currentPath / "child" / "node_modules" / "resolve-cc-file-json.json",
                  ModuleType::JSON);

        // A binary file exists.
        ResolveIt("resolve-cc-file-napa",
                  currentPath / "child" / "node_modules" / "resolve-cc-file-napa.napa",
                  ModuleType::NAPA);
        ...
        // A directory with index.js exists.
        ResolveIt("resolver-cc-js",
                  currentPath / "child" / "node_modules" / "resolver-cc-js" / "index.js",
                  ModuleType::JAVASCRIPT);

        // A directory with index.json exists.
        ResolveIt("resolver-cc-json",
                  currentPath / "child" / "node_modules" / "resolver-cc-json" / "index.json",
                  ModuleType::JSON);

        // A directory with index.napa exists.
        ResolveIt("resolver-cc-napa",
                  currentPath / "child" / "node_modules" / "resolver-cc-napa" / "index.napa",
                  ModuleType::NAPA);
    }
}
